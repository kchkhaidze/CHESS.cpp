#' @title ABC SMC with bulk tumour samples
#'
#' @description Performs ABC SMC simulations to recover a given parameter set
#'    for a tumour simulation using its bulk samples.
#'
...
#' @param nbulks Number of bulks to sample.
#' @param radius Bulk radius.
#' @param nbins Number of bins for a multivariate histogram construction.
...
#' @param target.data List of bulk samples (each element should be named as "mutation_data"; so an access call to a bulk sample element would be: bulk.data.list[[id]]$mutation_data). Each bulk sample should be formatted as a data.frame of unique mutations with the following columns: clone (Clone type label), alt (Number of reads), depth (Sequencing depth), id (Unique ID).
...
ABCSMCwithBulkSamples <- 
  ...
  if (sampling == 'punch') {
    sampling.strategy <- PrepareBiopsySquares(radius = radius,
                                              grid = grid,
                                              dimension = dimension)
  } else if (sampling == 'needle') {
    sampling.strategy <- PrepareBiopsyNeedles(radius = radius,
                                              grid = grid,
                                              nsamples = nbulks,
                                              dimension = dimension)
  } else {
    print('Unrecognised sampling strategy')
  }
  ...
  if (is.null(target.data)) {
    ...
    # SimulateTumourSample first simulates a tumour with a given parameter set,
    # then samples cells corresponding to a bulk shape coordinates
    # and adds noise model to mimic next generation sequencing procedure.
    # Returned is a list of sequenced mutations per bulk sample.
  ...
  # Get multivaraite distribution of the target tumour sample bulk VAFs
  target.multidistr <- GetMultivrtDistr(bulk.data.list = target.data,
                                        nbins = nbins,
                                        nbulks = nbulks)
    ...
    if (r == 1) {
      ...
      proposed.particles <- do.call(
        ...
        while (TRUE) {
          ...
          # Get multivaraite distribution of the sampled bulk VAFs
          sim.multidistr <- GetMultivrtDistr(
            bulk.data.list = sim.data, nbins = nbins, nbulks = nbulks)

          # Stack the obtained distributions for target and sim tumour samples
          comb.sim.target <- merge(
            target.multidistr, sim.multidistr, by = 'Var1', all = T)

          comb.sim.target <- data.frame(apply(
            comb.sim.target[, -1], 2, function(x) {x[is.na(x)] <- 0; x}))

          # Calculate Wasserstein distane between obtained frequency counts
          dist <- transport::wasserstein1d(
            comb.sim.target$Freq.x, comb.sim.target$Freq.y)
      ...
      perturb.mu <- sigma*(
        max(particles.perround[[r - 1]]$mu) - min(particles.perround[[r - 1]]$mu))
      perturb.t <- sigma*(
        max(particles.perround[[r - 1]]$t) - min(particles.perround[[r - 1]]$t))
      perturb.s <- sigma*(
        max(particles.perround[[r - 1]]$s) - min(particles.perround[[r - 1]]$s))
      perturb.d <- sigma*(
        max(particles.perround[[r - 1]]$d) - min(particles.perround[[r - 1]]$d))
      perturb.a <- sigma*(
        max(particles.perround[[r - 1]]$a) - min(particles.perround[[r - 1]]$a))
      ...
      while (nrow(proposed.particles) < N) {
        ...
        proposed.particles.part <- do.call(
          rbind, parallel::mclapply(1:nrunsleft, function(i) {
            ...
            # Sample particles from the previous round of particles' set,
            ...
            while (TRUE) {
              ...
              # Stack the obtained distributions for target and sim tum samples
              comb.sim.target <- merge(
                target.multidistr, sim.multidistr, by = 'Var1', all = T)
              ...
              # Calculate Wasserstein dist between distribution frequency counts
              potential.dist <- transport::wasserstein1d(
                comb.sim.target$Freq.x, comb.sim.target$Freq.y)
